...
def handle_storm_at_sea(
    ...
    result_text = f"The 2d6 result was {raw_result}"
    if evil_omens:
        result_text += f", reduced to {modified_result} by evil omens"
    result_text += "."

    if not fleets:
        Log.create_object(
            game.id, f"{prefix} {result_text} Rome had no fleets to lose."
        )
        return True

    if fleet_losses == 0:
        Log.create_object(game.id, f"{prefix} {result_text} No fleets were lost.")
        return True
    ...
    if fleet_losses == len(fleets):
        ...
        Log.create_object(
            game.id,
            f"{prefix} {result_text} {elimination_text}",
        )
    ...
    Log.create_object(
        game.id,
        f"{prefix} {result_text} The HRAO must choose {fleet_losses} Roman {fleet_noun} to eliminate.",
    )
    return False
